...
const ConsumerList = () => {
  ...
  useEffect(() => {
    ...
    if (!userInfo.isAdmin && !userInfo) {
      ...
      if (successCreate) {
        history.push(`/admin/productlist/san-pham-thien-van/${consumerProduct._id}/edit`);
      } else {
  ...
  return (
    <Container>
      <Row>
        <Col>
          <h1 style={{ marginBottom: "20px" }}>Sản phẩm thiên văn khác</h1>
        </Col>
          ...
          <tbody>
            {consumerProducts.map((consumer) => (
              <tr key={consumer._id}>
                ...
                <td>
                  <LinkContainer
                    to={`/admin/productlist/san-pham-thien-van/${consumer._id}/edit`}
                  >
